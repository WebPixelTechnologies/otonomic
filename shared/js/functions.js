...
(function($, window, document) {
    ...
    jQuery(function() {
        ...
        jQuery(document).on("click", ".modal_close_btn", function (e) {
            e.preventDefault();
            closeModalWindows();
            jQuery('.publish-modal').modal('hide');// had to duplicate the call to hide it, wasnt working otherwise
        });
        ...
        jQuery(document).on('click', "#facebook_fanpages_list a.page_name, #facebook_fanpages_list a.page_image", function (e) {
            ...
            redirect_user(BUILDER_PATH + page_id, "Creating a website for " + page_name + "...", 1000);
        });
...
function openFacebookCancelModal() {
    ...
    jQuery('#fbCancelModal').modal({
        keyboard:false
    });
}

function closeFacebookCancelModal() {
    jQuery('#fbCancelModal').modal('hide');
}

function openFacebookManagePagesRejectedModal() {
    jQuery('#fbManagePagesModal').modal({
        keyboard:false
    });
}

function closeFacebookManagePagesRejectedModal() {
    jQuery('#fbManagePagesModal').modal('hide');
}

function openFacebookManagePagesApprovedModal() {
    jQuery('#fbConnectedModal').modal({
        keyboard:false
    });
}

function closeFacebookManagePagesApprovedModal() {
    jQuery('#fbConnectedModal').modal('hide');
}

function closeModalWindows() {
    jQuery('.publish-modal').modal('hide');
}

function openThankYouModal() {
    jQuery("#thankYouModal").modal({ backdrop: 'static', keyboard: false },'show');
    jQuery("#thankYouModal").addClass('animateIn');
...
function checkAssignSiteToUser(user_id, access_token) {
    jQuery.post(BUILDER_PATH+"/users_checkFacebookLogin.json",
        {
            facebook_id: user_id,
...
function createPersonalSite() {
    ...
    redirect_user(BUILDER_PATH + '&type=fb_personal', "Creating your website...", 2000);
}
...
function createSiteSingleFanpageAutomatically(page_id) {
    ...
    redirect_user(BUILDER_PATH + page_id, "Creating your website...", 2000);
}
...
function getUserFacebookName(user_facebook_id){
    FB.api('/'+user_facebook_id, function(response) {
        jQuery(".facebook_user_name").html(response.name);
    });
}
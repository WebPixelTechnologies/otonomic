...
<div>
<?php
if(isset($_GET[mymail_option('cron_secret')])){
	...
	$new = array(
		'ip' => mymail_get_ip(),
		'timestamp' => time(),
		'oldtimestamp' => (isset($last['timestamp']) ? $last['timestamp'] : time()),
		'time' => 0,
		'timemax' => (isset($last['timemax']) ? $last['timemax'] : 0)
	);
	
	update_option('mymail_cron_lasthit', $new);
	...
	<?php
	$new['time'] = $time;
	$new['timemax'] = max($new['timemax'], $time);
	update_option('mymail_cron_lasthit', $new);

...
class mymail_settings {
	...
	public function __construct() {
		...
		add_action('wp_ajax_mymail_get_fallback_images', array( &$this, 'ajax_get_fallback_images'));
		...
		add_action('wp_ajax_mymail_get_system_info', array( &$this, 'ajax_get_system_info'));
	...
	public function reset_settings() {
		if(is_super_admin() && wp_verify_nonce( $_REQUEST['_wpnonce'], 'mymail-reset-settings' )){
			$purchasecode =	get_option( 'mymail_purchasecode_disabled' );
			$this->define_settings( false );
			update_option( 'mymail_purchasecode_disabled', !!$purchasecode);
	...
	public function verify($options) {
		...
		$verify = array('from', 'reply_to', 'homepage', 'trackcountries', 'trackcities', 'slug', 'hasarchive', 'custom_field', 'forms', 'form_css', 'send_at_once', 'send_delay', 'send_period', 'bounce', 'cron_service', 'cron_secret', 'interval', 'roles', 'tweet_cache_time', 'deliverymethod', 'smtp_host', 'smtp_port', 'bounce_check', 'bounce_delay', 'dkim_domain', 'dkim_selector', 'dkim_identity', 'dkim_passphrase', 'dkim_private_key', 'purchasecode');
			...
			switch($id){
				...
				case 'slug':
					if($old != $value){
						...
						add_settings_error( 'mymail_options', 'mymail_options', sprintf(__('Campaign slug changed! Please re-save your %s', 'mymail'), '<a href="options-permalink.php">'.__('Permalink Structure', 'mymail').'</a>'));
				...
				case 'hasarchive':
					...
					if($old != $value){
						add_settings_error( 'mymail_options', 'mymail_options', sprintf(__('Please re-save your %s', 'mymail'), '<a href="options-permalink.php">'.__('Permalink Structure', 'mymail').'</a>'));
						add_action( 'plugins_loaded', 'flush_rewrite_rules' );
					}
				break;
				...
				case 'smtp_port':
					...
					if($old != $value && function_exists( 'fsockopen' ) && $options['deliverymethod'] == 'smtp') :
	...
	public function ajax_get_fallback_images(){
		...
		foreach($images as $image){
			$return['images'] .= '<option value="'.$image->ID.'">['.$image->ID.'] '.$image->post_title.'</option>';
		}
		
		if($images) $return['images'] .= '<option value="0" data-offset="'.($offset+$load_at_once).'">'.__('load more', 'mymail').'</option>';
	...
	public function ajax_get_system_info(){
		...
		$space = 30;
		$infos = $this->get_system_info($space);
	...
	private function get_system_info($space = 30){
		...
		if($mail->send_notification( 'Sendtest', 'this testmessage can get deleted', array('notification' => ''), false )){
			$send_success = 'OK';
		}else{
		...
		$settings = array(
			...
			"MyMail Purchasecode" =>   mymail_option('purchasecode', 'Not defined'),
			'--',
			"Newsletter Homepage" =>   get_permalink( mymail_option('homepage') ),
			...
			"Cron URL" =>              MYMAIL_URI.'/cron.php?'.mymail_option('cron_secret'),

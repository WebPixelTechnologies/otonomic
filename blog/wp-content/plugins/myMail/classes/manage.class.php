...
class mymail_manage {
	...
	public function ajax_get_import_data() {
		...
		$html = '<h2>'.__('Select columns', 'mymail').'</h2><form id="subscriber-table"><table class="wp-list-table widefat">';
		...
		for($i = 0; $i < $cols; $i++){
			...
			foreach($meta as $key => $value){
				$is_selected = ((strip_tags($firstline[$i]) == __('registered', 'mymail') && $key == '_signupdate'));
				$select .= '<option value="'.$key.'" '.($is_selected ? 'selected' : '').'>'.$value.'</option>';
			}
			$select .= '</select>';
			$html .= '<td>'.$select.'</td>';
...
';
	...
	public function ajax_do_export() {
		...
		if(isset($d['nolists'])){
			...
			$sql = "SELECT a.ID, a.post_title FROM {$wpdb->posts} a LEFT JOIN {$wpdb->users} u ON (a.post_author = u.ID) WHERE a.post_status IN ('".implode("','", $d['status'])."') AND a.post_type IN('subscriber') AND a.ID NOT IN (SELECT object_id FROM {$wpdb->term_relationships} tr LEFT JOIN {$wpdb->term_taxonomy} tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE taxonomy = 'newsletter_lists') LIMIT $offset, $limit;";
		...
		if(!empty($d['lists'])){
			...
			$sql = "SELECT a.ID, a.post_title, a.post_status FROM {$wpdb->posts} a LEFT JOIN {$wpdb->term_relationships} b ON ( a.ID = b.object_id ) LEFT JOIN {$wpdb->postmeta} c ON ( a.ID = c.post_id ) LEFT JOIN {$wpdb->term_taxonomy} d ON ( d.term_taxonomy_id = b.term_taxonomy_id ) LEFT JOIN {$wpdb->terms} e ON ( e.term_id = d.term_id ) WHERE a.post_type IN('subscriber') AND a.post_status IN ('".implode("','", $d['status'])."') AND e.term_id IN (".implode(',', $d['lists']).") GROUP BY a.ID LIMIT $offset, $limit;";

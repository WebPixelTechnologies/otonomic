...
*/
	...
	public function share_service($url, $title = '' ) {
		...
		if($count = preg_match_all('#\{(share:('.$social.') ?([^}]+)?)\}#', $this->content, $hits)){
					...
					if ( false === $replace_to ) {
						...
						if(!$relative_to_absolute){
							...
							if(!empty($org_src)){
								...
								$replace_to = 'src="'.$img['url'].'" height="'.round($img['height']/$factor).'"';
					...
					if($post){
						...
						switch($what){
							...
							case 'modified_time_gmt':
								...
								break;
							case 'excerpt':
								$replace_to = strip_shortcodes((!empty($post->{'post_excerpt'}) ? $post->{'post_excerpt'} : wp_trim_words($post->{'post_content'})));
								break;
							case 'content':

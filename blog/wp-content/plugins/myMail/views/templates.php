...
<div id="current-template">
	...
	<div class="current-template">
		...
		<?php if(!in_array($name, array('index.html', 'notification.html')) && current_user_can('mymail_delete_templates')) : ?><a class="remove-file" data-file="<?php echo $slug .'/'.$name ?>" title="<?php echo sprintf(__('remove file %s', 'mymail'), $name ); ?>" data-confirm="<?php echo esc_html(sprintf(__('You are about to delete the file %s', 'mymail'), "'".$name."'")) ?>">&#10005;</a><?php endif; ?>
	</div>
...
<?php 
	...
	foreach($templates as $slug => $data){
		?>	
		<li class="available-template" id="mymail_template_<?php echo $i?>" data-id="<?php echo $i++?>">
			<?php if(isset($updates[$slug])){?>
				<span class="update-badge"><?php echo $updates[$slug]?></span>
			<?php }?>
			<a class="thickbox thickbox-preview screenshot" title="<?php echo $data['name'].' '.$data['version']?> <?php _e('by', 'mymail'); ?> <?php echo $data['author']?>" href="<?php echo $t->url .'/' .$slug .'/index.html'?>?preview_iframe=1&amp;TB_iframe=true&amp;width=700&amp;height=700">
				<img alt="" src="<?php echo $t->get_screenshot($slug)?>" width="300">
			</a>
			<h3><?php echo $data['name'].' '.$data['version']?></h3><div> <?php _e('by', 'mymail'); ?> <?php if(!empty($data['author_uri'])) : ?><a href="<?php echo $data['author_uri']?>"><?php echo $data['author']?></a><?php else : ?> <?php echo $data['author']?><?php endif; ?></div>

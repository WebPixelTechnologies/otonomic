...
<div id="misc-publishing-actions">
	...
	if ( !in_array( $post->post_status, array('publish', 'future', 'private', 'subscribed', 'unsubscribed', 'hardbounced') ) || 0 == $post->ID ) {
		if ( $can_publish ) :
			...
			<?php submit_button( __( 'Schedule', 'mymail' ), 'primary', 'publish', false, array( 'tabindex' => '5', 'accesskey' => 'p' ) ); ?>
	<?php	else : ?>
			<input name="original_publish" type="hidden" id="original_publish" value="<?php esc_attr_e('Publish', 'mymail') ?>" />
			<?php submit_button( __( 'Save', 'mymail' ), 'primary', 'publish', false, array( 'tabindex' => '5', 'accesskey' => 'p' ) ); ?>
	<?php	endif;
		else : ?>
			<input name="original_publish" type="hidden" id="original_publish" value="<?php esc_attr_e('Submit for Review', 'mymail') ?>" />
			<?php submit_button( __( 'Submit for Review', 'mymail' ), 'primary', 'publish', false, array( 'tabindex' => '5', 'accesskey' => 'p' ) ); ?>
	<?php
		endif;
	} else { ?>
		...
		<?php } ?>
			<input name="save" type="submit" class="button-primary" id="publish" tabindex="16" accesskey="p" value="<?php esc_attr_e('Update', 'mymail') ?>" />
	<?php
	} ?>
	</div>
<div class="clear"></div>

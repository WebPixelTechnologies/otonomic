<?php 
		global $wpdb, $mymail_templates, $mymail_options, $current_user, $wp_post_statuses, $mymail_capabilities, $wp_roles, $locale;
...
?>
	...
	<div id="tab-forms" class="tab">
		...
		<div id="formsnav" class="nav-tab-wrapper hide-if-no-js">
			...
			<a class="nav-tab<?php if(!$id) echo ' nav-tab-active"'; ?>" href="#form-tab-<?php echo $id?>"><?php echo $form['name'] ?></a>
			<?php }?>
			<a class="button button-primary" href="#" id="mymail_add_form"><?php _e('add form' ,'mymail') ?></a>
		</div>
		
			<?php foreach($forms as $id => $form){?>
		<div class="subtab form" id="form-tab-<?php echo $id?>" <?php if(!$id) echo 'style="display:block"'; ?>>

			<a href="#" class="mymail_remove_form" title="<?php _e('remove this form', 'mymail'); ?>">&#10005;</a>
			<table class="nested">
				<tr><td width="205"><?php _e('Name', 'mymail'); ?></td><td><input type="text" name="mymail_options[forms][<?php echo $form['id']; ?>][name]" class="widefat" value="<?php echo (isset($form['name'])) ? $form['name'] : __('Default Form' ,'mymail') ?>" placeholder="<?php _e('Name your form', 'mymail'); ?>"></td></tr>
				</td></tr>
				
				<tr><td>shortcode</td><td><code>[newsletter_signup_form id=<?php echo $form['id']; ?>]</code><input type="hidden" class="mymail_form_id" name="mymail_options[forms][<?php echo $form['id']; ?>][id]" value="<?php echo $form['id']; ?>"></td></tr>
				...
				foreach($form['order'] as $field){
					...
					?>
				<li>
				<input type="<?php echo $field == 'email' ? 'hidden' : 'checkbox'?>" name="mymail_options[forms][<?php echo $form['id']; ?>][order][]" class="form-order-check" value="<?php echo $field ?>" checked title="<?php _e('enable this field', 'mymail'); ?>"> <input class="label widefat" type="name" name="mymail_options[forms][<?php echo $form['id']; ?>][labels][<?php echo $field ?>]" value="<?php echo $label ?>" title="<?php _e('define a label for this field', 'mymail'); ?>" placeholder="<?php echo $fields[$field] ?>">
				...
				foreach($fields as $field => $name){
					...
					?>
				<li class="inactive">
				<input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][order][]" class="form-order-check" value="<?php echo $field ?>" title="<?php _e('enable this field', 'mymail'); ?>"> <input class="label widefat" type="name" name="mymail_options[forms][<?php echo $form['id']; ?>][labels][<?php echo $field ?>]" value="<?php echo $label ?>" title="<?php _e('define a label for this field', 'mymail'); ?>" placeholder="<?php echo $name ?>">
				...
				<tr><td><?php _e('submit button label', 'mymail'); ?></td><td><input type="text" name="mymail_options[forms][<?php echo $form['id']; ?>][submitbutton]" class="widefat" value="<?php if(isset($form['submitbutton'])) echo $form['submitbutton']; ?>" placeholder="<?php echo mymail_text('submitbutton')?>" ></td></tr>
				</td></tr>
				
				<tr><td><?php _e('show asterisk', 'mymail'); ?></td><td><label><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][asterisk]" value="1" <?php if(isset($form['asterisk'])) if($form['asterisk']) echo ' checked'; ?>> <?php _e('show asterisk on required fields', 'mymail'); ?></label>
				</td></tr>
				
				<tr><td><?php _e('offer lists', 'mymail'); ?></td><td><label><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][userschoice]" class="mymail_userschoice" value="1" <?php checked(isset($form['userschoice'])); ?>> <?php _e('users decide which list they subscribe to', 'mymail'); ?></label>
				<br> &nbsp; <label> <input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][dropdown]" class="mymail_dropdown" value="1" <?php checked(isset($form['dropdown'])) ?><?php disabled(!isset($form['userschoice'])) ?>> <?php _e('show drop down instead of check boxes', 'mymail'); ?></label>
				</td></tr>
			...
			if(!empty($lists)){
				...
				?><br><label> <input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][precheck]" class="mymail_precheck" value="1" <?php checked(isset($form['precheck'])) ?><?php disabled(!isset($form['userschoice'])) ?>> <?php _e('checked by default', 'mymail'); ?></label>
				<?php 
			}else{
				?>
				<p class="description"><?php _e('No Lists found!' ,'mymail'); echo ' <a href="edit-tags.php?taxonomy=newsletter_lists&post_type=newsletter">'.__('Create a List now' ,'mymail').'</a>';?></p>	
			...
			?>
				...
				<tr><td><?php _e('new lists', 'mymail'); ?></td><td><label><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][addlists]" value="1" <?php checked(isset($form['addlists'])) ?>> <?php _e('assign new lists automatically to this form', 'mymail'); ?></label></td></tr>
				
				<tr><td><?php _e('inline labels', 'mymail'); ?></td><td><label><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][inline]" value="1" <?php checked(isset($form['inline'])) ?>> <?php _e('place labels inside input fields', 'mymail'); ?></label></td></tr>
				
				<tr><td><?php _e('pre-fill fields', 'mymail'); ?></td><td><label><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][prefill]" value="1" <?php checked(isset($form['prefill'])) ?>> <?php _e('fill fields with known data if user is logged in', 'mymail'); ?></label></td></tr>
				
				<tr>
					<td><?php _e('Double-Opt-In' ,'mymail') ?></td>
					<td><label><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][double_opt_in]" class="double-opt-in" data-id="<?php echo $form['id']; ?>" value="1" <?php checked(isset($form['double_opt_in'])) ?>> <?php _e('new subscribers must confirm their subscription' ,'mymail') ?></label>
					</td>
				</tr>
			</table>
			
				<div id="double-opt-in-field-<?php echo $form['id']; ?>" class="double-opt-in-field" <?php if(!isset($form['double_opt_in'])) echo ' style="display:none"';?>>
					...
					<table class="nested">
						...
						<tr>
							...
							<table class="form-table">
								<tr valign="top">
									...
									<td><input type="text" id="mymail_text_subscription_subject" name="mymail_options[forms][<?php echo $form['id']; ?>][text][subscription_subject]" value="<?php echo esc_attr(isset($form['text']['subscription_subject']) ? $form['text']['subscription_subject'] : ''); ?>" class="regular-text"></td>
								</tr>
								<tr valign="top">
									<td scope="row"><label for="mymail_text_subscription_headline_<?php echo $form['id']; ?>"><?php _e('Headline', 'mymail'); ?>: <code>{headline}</code></label></td>
									<td><input type="text" id="mymail_text_subscription_headline_<?php echo $form['id']; ?>" name="mymail_options[forms][<?php echo $form['id']; ?>][text][subscription_headline]" value="<?php echo esc_attr(isset($form['text']['subscription_headline']) ? $form['text']['subscription_headline'] : ''); ?>" class="regular-text"></td>
								</tr>
								<tr valign="top">
									<td scope="row"><label for="mymail_text_subscription_link_<?php echo $form['id']; ?>"><?php _e('Linktext', 'mymail'); ?>:</label> <code>{link}</code></td>
									<td><input type="text" id="mymail_text_subscription_link_<?php echo $form['id']; ?>" name="mymail_options[forms][<?php echo $form['id']; ?>][text][subscription_link]" value="<?php echo esc_attr(isset($form['text']['subscription_link']) ? $form['text']['subscription_link'] : ''); ?>" class="regular-text"></td>
								</tr>
								<tr valign="top">
									<td scope="row"><label for="mymail_text_subscription_text_<?php echo $form['id']; ?>"><?php _e('Text', 'mymail'); ?>: <code>{content}</code></label><p class="description"><?php echo sprintf(__('The text new subscribers get when Double-Opt-In is selected. Use %s for the link placeholder. Basic HTML is allowed' ,'mymail'),'<code>{link}</code>'); ?></p></td>
									<td><textarea id="mymail_text_subscription_text_<?php echo $form['id']; ?>" name="mymail_options[forms][<?php echo $form['id']; ?>][text][subscription_text]" rows="10" cols="50" class="large-text"><?php echo esc_attr(isset($form['text']['subscription_text']) ? $form['text']['subscription_text'] : ''); ?></textarea></td>
								</tr>
							</table>
							</td>
						</tr>
						...
						<tr>
							...
							<td><div><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][subscription_resend]" value="1" <?php checked(isset($form['subscription_resend'])) ?>> <?php echo sprintf(__('resend confirmation %1$s times with a delay of %2$s hours if user hasn\'t confirmed the subscription' ,'mymail'), '<input type="text" name="mymail_options[forms]['.$form['id'].'][subscription_resend_count]" value="'.esc_attr($form['subscription_resend_count']).'" class="small-text">', '<input type="text" name="mymail_options[forms]['.$form['id'].'][subscription_resend_time]" value="'.esc_attr($form['subscription_resend_time']).'" class="small-text">') ?></div></td>
						</tr>
						
						<tr><td><?php _e('redirect after confirm', 'mymail'); ?></td><td><input type="text" name="mymail_options[forms][<?php echo $form['id']; ?>][confirmredirect]" class="widefat" value="<?php if(isset($form['confirmredirect'])) echo $form['confirmredirect']; ?>" placeholder="http://www.example.com" ></td>
						</tr>
						<tr>
							<td><?php _e('Attach vCard' ,'mymail') ?></td>
							<td><label><input type="checkbox" name="mymail_options[forms][<?php echo $form['id']; ?>][vcard]" class="vcard" value="1" <?php checked(isset($form['vcard'])); ?> data-id="<?php echo $form['id']; ?>"> <?php _e('attach vCard to all confirmation mails' ,'mymail') ?></label></td>
						</tr>
						
						<tr>
							<td colspan="2">
							<div id="vcard-field-<?php echo $form['id']; ?>" <?php if(!isset($form['vcard'])) echo ' style="display:none"';?> class="vcard-field">
							<table class="form-table">
								<tr valign="top">
									<td scope="row" width="200"><?php _e('vCard file name', 'mymail'); ?></td>
									<td><input type="text" name="mymail_options[forms][<?php echo $form['id']; ?>][vcard_filename]" value="<?php echo esc_attr(!empty($form['vcard_filename']) ? $form['vcard_filename'] : 'vCard.vcf'); ?>" class="regular-text"></td>
								</tr>
								<tr valign="top">
									...
									<td><?php $vcard = isset($form['vcard_content']) ? $form['vcard_content'] : ''; if(empty($vcard)) $vcard = $this->get_vcard(); ?><textarea name="mymail_options[forms][<?php echo $form['id']; ?>][vcard_content]" rows="10" cols="50" class="large-text code"><?php echo esc_attr($vcard); ?></textarea></td>
								</tr>
							</table>
							</div>
							</td>
				...
				<tr><td width="205"><?php _e('redirect after submit', 'mymail'); ?></td><td><input type="text" name="mymail_options[forms][<?php echo $form['id']; ?>][redirect]" class="widefat" value="<?php if(isset($form['redirect'])) echo $form['redirect']; ?>" placeholder="http://www.example.com" ></td></tr>
				...
				<tr class="embed-form-options"><td colspan="2">
					...
					<p><strong><?php _e('HTML version', 'mymail'); ?></strong> <span class="description"><?php _e('save before you use it!', 'mymail'); ?></span><textarea class="widefat code form-output"><?php echo mymail_form( $form['id'], 100, true, 'extern' )?></textarea></p>
				</td></tr>
	...
	<table class="form-table">
		<tr valign="top">
			...
			?>
			<p class="description"><?php echo sprintf(__('You can still send %1$s mails within the next %2$s', 'mymail'), '<strong>'.$mails_left.'</strong>' , '<strong title="'.date_i18n(get_option('date_format').' '.get_option('time_format'), $next_reset+(get_option('gmt_offset')*3600), true).'">'.human_time_diff($next_reset).'</strong>'); ?> &ndash; <a href="<?php echo add_query_arg('resetlimits', 1) ?>"><?php _e('reset these limits', 'mymail'); ?></a></p>
	...
	<div id="tab-system_info" class="tab">
		...
		<textarea id="system_info_content" readonly class="code" onclick="this.focus();this.select()" title="<?php _e( 'To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac).', 'mymail' ); ?>">
		</textarea>
	...
	<?php if(is_super_admin()) :?>
		<p class="resetbutton">
			<a onclick='return confirm("<?php _e('Do you really like to reset the options? This cannot be undone!', 'mymail'); ?>");' href="options-general.php?page=newsletter-settings&reset=1&_wpnonce=<?php echo wp_create_nonce('mymail-reset-settings') ?>"><?php _e('Reset all settings', 'mymail'); ?></a>
		</p>
	<?php endif;?>

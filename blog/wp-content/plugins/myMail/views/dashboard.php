...
<div class="stats table_content <?php if($camps[0]->post_status == 'active') echo "isactive";?>" id="stats_cont">
	<p class="sub"><?php _e('Recent Campaign', 'mymail') ?> <a class="prev_camp disabled"></a> <a id="camp_name" href="post.php?post=<?php echo $camps[0]->ID ?>&action=edit" title="<?php _e('edit', 'mymail')?>"><?php echo $camps[0]->post_title?></a> <a class="next_camp<?php if(count($camps) <= 1) echo ' disabled';?>"></a></p>
...
<table id="stats">
	...
	<tr>
	<td align="center"><span class="verybold" id="stats_total"><?php echo ($campaign_data['sent'])?></span></td>
	<td width="100">
	<div id="stats_open" class="piechart" data-percent="<?php echo !empty($campaign_data['sent']) ? $campaign_data['opens']/$campaign_data['sent']*100 : 0 ?>"><span>0</span>%</div>
	</td>
	<td width="100" align="center">
	<div id="stats_clicks" class="piechart" data-percent="<?php echo !empty($campaign_data['opens']) ? $campaign_data['totaluniqueclicks']/$campaign_data['opens']*100 : 0 ?>"><span>0</span>%</div>
	</td>
	<td width="100" align="center">
	<div id="stats_unsubscribes" class="piechart" data-percent="<?php echo !empty($campaign_data['opens']) ? $campaign_data['unsubscribes']/$campaign_data['opens']*100 : 0 ?>"><span>0</span>%</div>
	</td>
	<td width="100" align="center">
	<div id="stats_bounces" class="piechart" data-percent="<?php echo $campaign_data['hardbounces']/(($campaign_data['sent'] ? $campaign_data['sent'] : 1)+$campaign_data['hardbounces'])*100 ?>"><span>0</span>%</div></td>
	</tr>
</table>
...
<div class="table table_content">
	<p class="sub"><?php
		...
		(<a href="post-new.php?post_type=newsletter"><?php _e('new', 'mymail') ?></a>)
	</p>
	<?php if($total_camps):?>
	<table>
		<tbody>
...
<div class="table table_content">
	...
	<table>
		...
		<?php if($subscribers->subscribed){?>
			<tr><td class="b"><a href="edit.php?post_status=subscribed&post_type=subscriber"><?php echo number_format_i18n($subscribers->subscribed) ?></a></td><td class="t"><a href="edit.php?post_status=subscribed&post_type=subscriber"><?php echo sprintf( _n( '%n Active Subscriber', '%n Active Subscribers', $subscribers->subscribed, 'mymail'), count($subscribers)) ?></a></td></tr>
		<?php }?>
		<?php if($subscribers->unsubscribed){?>
			<tr><td class="b"><a href="edit.php?post_status=unsubscribed&post_type=subscriber"><?php echo number_format_i18n($subscribers->unsubscribed) ?></a></td><td class="t"><a href="edit.php?post_status=unsubscribed&post_type=subscriber"><?php echo sprintf( _n( '%n Unsubscribed Subscriber', '%n Unsubscribed Subscribers', count($subscribers), 'mymail'), $subscribers->unsubscribed) ?></a></td></tr>
		<?php }?>
		<?php if($subscribers->hardbounced){?>
			<tr><td class="b"><a href="edit.php?post_status=hardbounced&post_type=subscriber"><?php echo number_format_i18n($subscribers->hardbounced) ?></a></td><td class="t"><a href="edit.php?post_status=hardbounced&post_type=subscriber"><?php echo sprintf( _n( '%n Hardbounced Subscriber', '%n Hardbounced Subscribers', count($subscribers), 'mymail'), $subscribers->hardbounced) ?></a></td></tr>
		<?php }?>
		<?php if($confirms = get_option( 'mymail_confirms' )){?>
			<tr><td class="b"><br><a href="edit.php?post_type=newsletter&page=mymail_subscriber-manage&tab=pending"><?php echo number_format_i18n(count($confirms)) ?></a></td><td class="t"><br><a href="edit.php?post_type=newsletter&page=mymail_subscriber-manage&tab=pending"><?php echo sprintf( _n( '%n Pending Subscriber', '%n Pending Subscribers', count($confirms), 'mymail'), count($confirms)) ?></a></td></tr>
		<?php }?>
	...
	if(!empty($counts)) : 
		...
		jQuery(document).ready(function($) {
			var data = google.visualization.arrayToDataTable([
				['<?php _e('Date', 'mymail') ?>','<?php _e('New unsubscribers', 'mymail') ?>','<?php _e('New subscribers', 'mymail') ?>','<?php _e('Clicks', 'mymail') ?>'],
...
<div class="versions">
	...
	?>
	<a href="update.php?action=upgrade-plugin&plugin=<?php echo urlencode(MYMAIL_SLUG);?>&_wpnonce=<?php echo wp_create_nonce('upgrade-plugin_' . MYMAIL_SLUG)?>" class="button"><?php printf( __('Update to %s', 'mymail'), $plugins->response[MYMAIL_SLUG]->new_version ? $plugins->response[MYMAIL_SLUG]->new_version : __( 'Latest', 'mymail' ) )?></a>
	<?php 
		}
	}

...
class Mixin_NextGen_Pro_Checkout extends Mixin
{
	...
	function enqueue_static_resources()
	{
		...
		wp_enqueue_style('ngg-pro-checkout', $this->get_static_url('photocrati-nextgen_pro_lightbox#checkout.css'));
	...
	function checkout_form()
	{
		...
		return $this->render_partial('photocrati-nextgen_pro_lightbox#ecommerce/checkout_form', array(
			'buttons'		=>	$buttons,
    ...
    function create_order($cart, $customer_name, $email, $total_amount, $payment_gateway, $shipping_street_address=NULL, $shipping_city=NULL, $shipping_state=NULL, $shipping_zip=NULL, $shipping_country=NULL, $status='verified')
    {
    ...
    function _send_email($order_hash, $subject, $body, $to=NULL)
    {
        ...
        if (($order = C_Order_Mapper::get_instance()->find_by_hash($order_hash))) {
            ...
            $order->total_amount        = M_NextGen_Pro_Lightbox::get_formatted_price($order->total_amount, FALSE, FALSE);
    ...
    function redirect_to_thank_you_page($order_hash, $send_emails=TRUE)
    {

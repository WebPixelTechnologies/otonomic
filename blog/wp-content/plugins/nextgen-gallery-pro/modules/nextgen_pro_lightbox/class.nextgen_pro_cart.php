...
class C_NextGen_Pro_Cart
{
    ...
    function to_array()
    {
        ...
        $total      = bcadd($subtotal, $shipping, $currency['exponent']);
    ...
    function get_subtotal()
    {
        $retval = 0;
        $settings = C_NextGen_Settings::get_instance();
        $exponent = C_NextGen_Pro_Currencies::$currencies[$settings->ecommerce_currency]['exponent'];

        foreach ($this->_state as $image_id => $image) {
            foreach ($image->items as $source => $pricelists) {
                foreach ($pricelists as $pricelist_id => $items) {
                    foreach ($items as $item_id => $item) {
                        $retval = bcadd($retval, bcmul($item->price, $item->quantity, $exponent), $exponent);
                    }
                }
            }
        }
        ...
        foreach ($consolidated as $pricelist_id => $source_array) {
            foreach ($source_array as $source => $items) {
                ...
                if (($shipping_klass = $sources->get($source, 'shipping_method'))) {
                    ...
                    $subtotal = 0;
                    foreach ($items as $item_id => $item) {
                        $subtotal = bcadd($subtotal, bcmul($item->price, $item->quantity, $exponent), $exponent);
                    }

                    // Calculate the shipping cost for local orders
                    $local_rate = 0;
                    if ($settings['domestic_shipping_method'] == 'flat') {
                        $local_rate = bcadd($local_rate, $settings['domestic_shipping_rate'], $exponent);
                    }
                    else {
                        $local_rate = bcadd($local_rate, bcmul($settings['domestic_shipping_rate'], ($subtotal/100), $exponent), $exponent);
                    }

                    // Calculate the shipping cost for international orders
                    $global_rate = 0;
                    if ($settings['global_shipping_method'] == 'flat') {
                        $global_rate = bcadd($global_rate, $settings['global_shipping_rate'], $exponent);
                    }
                    else {
                        $global_rate = bcadd($global_rate, bcmul($settings['global_shipping_rate'], ($subtotal/100), $exponent), $exponent);
                    }

                    // Determine what rate to use. The local rate is used as a minimum rate as well
                    if ($use_home_country) $retval = bcadd($retval, $local_rate, $exponent);
                    else if ($local_rate > $global_rate) $retval = bcadd($retval, $local_rate, $exponent);
                    else $retval = bcadd($retval, $global_rate, $exponent);
                }
            }
        }
    ...
    function get_total($use_home_country=TRUE)
    {
        $settings = C_NextGen_Settings::get_instance();
        $exponent = C_NextGen_Pro_Currencies::$currencies[$settings->ecommerce_currency]['exponent'];
        return bcadd($this->get_shipping($use_home_country), $this->get_subtotal(), $exponent);
    }

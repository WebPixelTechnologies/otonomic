(function($){
    ...
    Ngg_Pro_Cart.Views.Item_Row						= Ngg_Pro_Cart.Views.TemplateView.extend({
        ...
        update_subtotal: function(){
            this.$el.find('.subtotal_column').html(sprintf(Ngg_Pro_Cart_Settings.currency_format, this.model.subtotal()));
        },
        ...
        render: function(){
            ...
            quantity_field.on('change', function(e){
                _this.quantity_changed(this);
            });
    ...
    Ngg_Pro_Cart.Views.Cart							= Backbone.View.extend({
        ...
        update_totals: function(urgent){
            ...
            this.$el.find('#shipping_field').html(sprintf(Ngg_Pro_Cart_Settings.currency_format, this.model.shipping));
        ...
        render: function(){
            ...
            if (this.model.allow_international_shipping) {
                this.$el.find('#ship_to_row').show();
                this.$el.find('#ship_to_field select').change(function(){
                    _this.$el.find('#shipping_field').text('Calculating...');
                    _this.model.use_home_country =  $(this).val();
                    _this.model.recalculate_shipping();
                });
            }
            else {
                this.$el.find('#ship_to_row').hide();
            }

            // Display images
            this.model.each(function(image){
                var images_table = _this.$el.find('.ngg_pro_cart_images');
    ...
    Ngg_Pro_Cart.Views.Add_To_Cart = Backbone.View.extend({
        ...
        sanitize_quantity: function(e){
            var $this = $(e.target);
            if (!(e.keyCode == 8 || e.keyCode == 9 || e.keyCode == 46 || (e.charCode >= 48 && e.charCode <= 57))) {
                e.preventDefault();
                return false;
            }
            return true;
        ...
        update_cart_summary: function(){
            this.$el.find('.cart_count').text(Ngg_Pro_Cart.get_instance().item_count() + ' items');
            this.$el.find('.cart_total').html(sprintf(Ngg_Pro_Cart_Settings.currency_format, Ngg_Pro_Cart.get_instance().subtotal()));
        },

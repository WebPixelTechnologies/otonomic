...
jQuery(document).ready(function($){
	...
	$('.form-search').hover(function() { $(this).focus(); 
	});
    ...
    $(".searchfb").keyup(function () {        
        ...
        if (value.length >= minlength ) {
			...
			setTimeout(function(){
				...
				p2sTrack("Landing Page","Search",value);
				},2000);
                ...
                success : function (json, textStatus, xOptions) {
                   if(this_req_no < total_req_no) return;
            		...
            		items.push('<div class="media search-results-item">' + 
                                            '<a class="pull-left fanpage" target="_blank" href="http://builder.verisites.com/sites/add/fbid:' + val.id + '" target="_blank" title="Click to view site">' +
                                                    '<img class="media-object" src="http://graph.facebook.com/' + val.id + '/picture?type=square">' +
                                            '</a>' +
                                            '<div class="media-body pull-left">' + 
                                                    '<p class="media-heading">' +
                                                            '<a class="page_name" target="_blank" href="http://builder.verisites.com/sites/add/fbid:' + val.id + '" target="_blank" title="Click to view site">' + val.name + '</a>' +
                                                    '</p>' +
